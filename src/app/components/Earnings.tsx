...
import { TrendingUp, DollarSign, Calendar, BarChart2, Sparkles, Filter, CalendarDays, X } from "lucide-react";
  ...
  return (
    <div className="p-5 flex flex-col gap-5">
        ...
        <div className="relative" ref={filterRef}>
          <button
            ...
            className={`flex items-center gap-1.5 border rounded-xl px-3 py-1.5 text-[11px] font-medium transition-colors ${
              isFiltered
                ? "border-[#3B5BFF] bg-[#EEF1FE] text-[#3B5BFF]"
                : "border-border text-muted-foreground hover:bg-muted/50"
            }`}>
            <Filter size={11} />
            Filter
            {isFiltered && (
              <span className="w-4 h-4 rounded-full bg-[#3B5BFF] text-white text-[8px] font-bold flex items-center justify-center">1</span>
            )}
          </button>
            ...
            <div className="absolute right-0 top-full mt-2 z-30 w-72 bg-card rounded-2xl border border-border shadow-xl p-4 space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-[12px] font-bold text-foreground flex items-center gap-1.5">
                  <CalendarDays size={13} className="text-[#3B5BFF]" /> Date Range
                </span>
                {isFiltered && (
                  <button
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  className="flex-1 py-2 rounded-xl text-[11px] font-medium border border-border text-muted-foreground hover:bg-muted/50 transition-colors">
                  Cancel
                </button>
                <button
                  onClick={() => { setAppliedFrom(dateFrom); setAppliedTo(dateTo); setFilterOpen(false); }}
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <KpiCard title="Total Revenue"   value={fmtFull(Math.round(totalRevenue))}       pct={revenueGrowth}                   icon={DollarSign} accent="#3B5BFF"/>
